...
데이터를 새로 넣는 것이 아니라 **기존 행의 일부 값을 바꿀 때**는 `UPDATE`를 사용한다.

내가 기억한 핵심은 `SET`에서 바꿀 내용을 정하고, `WHERE`에서 **어느 행을 바꿀지 제한한다는 것**이다.
...
여러 컬럼을 한 번에 바꾸는 것도 가능하다.

```sql
SET 컬럼1 = 값1, 컬럼2 = 값2
```

여기서 가장 조심해야 할 부분은 `WHERE`다.  
조건을 빠뜨리면 특정 상품 하나가 아니라 **테이블 전체 행이 수정될 수 있다.**

그래서 수정하기 전에는 같은 조건으로 먼저 조회해보는 습관이 안전하다.
...
MySQL에는 실수로 많은 데이터를 바꾸는 상황을 줄이기 위한 안전 업데이트 설정이 있다.

현재 설정값은 다음처럼 확인할 수 있다.

```sql
SELECT @@SQL_SAFE_UPDATES;
```

값이 `1`이면 활성화된 상태이고, `0`이면 비활성화된 상태다.

안전 업데이트가 켜진 환경에서는 키 컬럼을 이용한 조건 없이 수정하거나 삭제하려 할 때 실행이 막힐 수 있다.  
`LIMIT`로 대상 수를 제한하는 방식도 사용할 수 있다.

예를 들어 이름만 조건으로 잡은 수정문은 환경 설정에 따라 안전 업데이트에 걸릴 수 있다.
...
세션에서 설정을 바꾸는 형태는 다음 정도로 기억해두면 된다.
...
강의에서 다룬 범위 기준으로 정리하면, MySQL 서버나 일반 애플리케이션 연결에서는 기본적으로 꺼져 있을 수 있고 MySQL Workbench는 실수를 줄이기 위해 켜서 사용하는 경우가 있다.

---

## 12. 행을 지우는 경우

특정 데이터를 테이블에서 제거할 때는 `DELETE`를 사용한다.
...
`UPDATE`와 마찬가지로 여기서도 `WHERE`가 중요하다.  
조건이 없으면 일부 데이터만 지우는 것이 아니라 **테이블의 모든 행이 삭제 대상이 된다.**

따라서 삭제 전에도 먼저 같은 조건으로 조회해서 대상이 맞는지 확인하는 편이 안전하다.

### `DELETE`와 `TRUNCATE` 구분

둘 다 데이터를 없앨 수 있지만 용도가 다르다.

- `DELETE`는 조건을 붙여 필요한 행만 지울 수 있다.
- `TRUNCATE`는 조건 없이 테이블 안의 데이터를 한 번에 비운다.
- `DELETE`로 전체 행을 지워도 `AUTO_INCREMENT` 값은 그대로 이어진다.
- `TRUNCATE`를 사용하면 `AUTO_INCREMENT` 값도 초기화된다.
- 강의에서 다룬 기준으로 `DELETE`는 트랜잭션 안에서 `ROLLBACK`을 고려할 수 있지만, `TRUNCATE`는 같은 방식으로 되돌리는 명령으로 보지 않는다.

내 기준에서는 **일부 행을 골라 지우는 작업은 `DELETE`, 테이블 내용 전체를 초기화하는 작업은 `TRUNCATE`**로 구분해서 기억하는 것이 가장 편했다.

---

## 13. 제약 조건이 실제로 막아주는 것

테이블을 만들 때 지정한 제약 조건은 단순한 설명이 아니라,  
데이터를 넣거나 바꿀 때 **허용되지 않는 값을 데이터베이스가 직접 거절하게 만드는 규칙**이다.

예제를 다시 확인하기 위해 관련 테이블의 데이터를 비우는 과정에서는 외래 키 검사를 잠시 끄는 형태를 사용할 수 있다.
...
`NOT NULL`이 설정된 컬럼은 값이 반드시 필요하다.

예를 들어 `name`이 필수인데 값을 주지 않고 회원 데이터를 넣으려 하면 저장되지 않는다.

```sql
INSERT INTO 테이블명 (필수컬럼을_제외한_컬럼들)
VALUES (...);
```

이 상황은 해당 컬럼에 넣을 값도 없고 기본값도 없기 때문에 오류가 발생하는 경우로 이해했다.

즉 `NOT NULL`은 **비어 있으면 안 되는 데이터**를 데이터베이스 단계에서 막는 역할을 한다.

### 중복되면 안 되는 값

`UNIQUE`가 붙은 컬럼에는 같은 값을 다시 넣을 수 없다.

예를 들어 이메일에 `UNIQUE`가 설정되어 있다면 이미 등록된 이메일을 다른 행에 다시 저장하려는 시도는 거절된다.

```sql
INSERT INTO 테이블명 (..., 중복불가컬럼, ...)
VALUES (..., 이미있는값, ...);
```

이 제약 조건은 이메일처럼 **행마다 서로 달라야 하는 값**을 유지하는 데 사용한다.

### 연결 대상이 실제로 존재해야 하는 경우

외래 키가 설정된 컬럼은 아무 숫자나 넣는 식으로 사용할 수 없다.

`orders`가 고객과 상품을 참조한다면 주문을 저장할 때 사용한 `customer_id`와 `product_id`가 각각 부모 테이블에 실제로 존재해야 한다.

```sql
INSERT INTO 자식테이블 (외래키컬럼, ...)
VALUES (참조값, ...);
```

참조 대상이 모두 존재하면 주문을 추가할 수 있고,  
둘 중 하나라도 없다면 외래 키 조건 때문에 저장이 실패할 수 있다.

여기서 내가 정리한 핵심은 **외래 키가 테이블 사이의 연결을 단순히 표시하는 것이 아니라, 존재하지 않는 대상을 참조하는 데이터까지 막아준다는 점**이다.
